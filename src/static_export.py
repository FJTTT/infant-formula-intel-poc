...
def main() -> None:
    ...
    payload = {
        ...
        "visual_assets": load_csv("visual_assets.csv"),
    }
...
<body>
  ...
  <main>
    ...
    <div class="metrics">
      ...
      <div class="metric">Visual/LP<strong id="visualCount">0</strong></div>
    </div>
    <div class="tabs">
      ...
      <button data-tab="visual">Visual/LP</button>
    ...
    <section id="visual"><div class="table-wrap"><table></table></div></section>
  ...
  <script>
    ...
    const filterDefs = [['company','メーカー'], ['brand','ブランド'], ['product','商品'], ['age_segment','対象年齢'], ['month','月'], ['event_type','Event Type']];
    ...
    (data.visual_assets || []).forEach(v => v.month = '');
    function values(key) {
    ...
    function buildFilters() {
      ...
      filterDefs.forEach(([key,label]) => {
        ...
        sel.innerHTML = '<option value="">All</option>' + values(key).map(v => `<option>${esc(v)}</option>`).join('');
    ...
    function pass(row, includeEventType=false) {
      for (const key of ['company','brand','product','age_segment']) if (state[key] && row[key] !== state[key]) return false;
      if (state.month && row.month && row.month !== state.month) return false;
    ...
    function table(id, rows, cols) {
      ...
      const body = rows.map(r => '<tr>' + cols.map(c => {
        const val = r[c] || '';
        if ((c === 'url' || c === 'page_url' || c === 'asset_url') && val) return `<td><a href="${esc(val)}" target="_blank" rel="noreferrer">${esc(val)}</a></td>`;
    ...
    function renderChart(events) {
      ...
      document.getElementById('chart').innerHTML = Object.entries(grouped).sort().map(([m,c]) =>
        `<div class="bar"><strong>${esc(m)}</strong><div class="fill" style="width:${Math.max(4, c / max * 100)}%"></div><span>${c}</span></div>`
      ).join('') || '該当なし';
    }
    function render() {
      ...
      const visual = (data.visual_assets || []).filter(v => pass(v)).filter(v => ["landing_page","og_image","twitter_image","banner_image"].includes(v.asset_type) || Number(v.confidence || 0) >= 0.7);
      docCount.textContent = docs.length; eventCount.textContent = events.length; campaignCount.textContent = campaigns.length; visualCount.textContent = visual.length;
      ...
      table('visual', visual, ['company','brand','product','asset_type','is_landing_page_candidate','confidence','creative_text','alt_text','page_url','asset_url']);
      renderChart(events);
    }
